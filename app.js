...
async function getVisibleGroups(){ const snap=await db.collection('groups').get(); const all=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>a.name.localeCompare(b.name)); if(currentProfile.role==='admin') return all; return all.filter(g=>(currentProfile.groups||[]).includes(g.name)); }
...
async function renderReports(){ const groups=await getVisibleGroups(); viewEl().innerHTML=`<section class="card"><h1>Reportes</h1><div class="row"><select id="rgrp"><option value="">Todos</option>${groups.map(g=>`<option>${safe(g.name)}</option>`).join('')}</select><input id="rdate" type="date" value="${today()}"><button id="loadReport">Consultar</button><button id="csv" class="ghost">Exportar CSV</button></div><div id="reportOut"></div></section>`; let last=[]; document.getElementById('loadReport').onclick=async()=>{ const g=document.getElementById('rgrp').value; const d=document.getElementById('rdate').value; let q=db.collection('attendance').where('date','==',d); const snap=await q.get(); last=snap.docs.map(x=>x.data()).filter(r=>!g||r.group===g); document.getElementById('reportOut').innerHTML=`<table><thead><tr><th>Fecha</th><th>Grupo</th><th>Alumno</th><th>Matrícula</th><th>Hora</th><th>Estado</th><th>Materia</th><th>Maestro</th></tr></thead><tbody>${last.map(r=>`<tr><td>${safe(r.date)}</td><td>${safe(r.group)}</td><td>${safe(r.studentName)}</td><td>${safe(r.matricula)}</td><td>${safe(r.time)}</td><td>${safe(r.status)}</td><td>${safe(r.subject)}</td><td>${safe(r.teacherName)}</td></tr>`).join('')}</tbody></table>`; }; document.getElementById('csv').onclick=()=>downloadCSV(last); }
function downloadCSV(rows){ const headers=['fecha','grupo','alumno','matricula','hora','estado','materia','maestro']; const csv=[headers.join(',')].concat(rows.map(r=>[r.date,r.group,r.studentName,r.matricula,r.time,r.status,r.subject,r.teacherName].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(','))).join('\n'); const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='asistencia.csv'; a.click(); }
