...
async function renderReports(){
  const groups=await getVisibleGroups();
  viewEl().innerHTML=`<section class="card"><h1>Reportes</h1>
  ...
  const loadRange=async()=>{
    ...
    const rows=students.map(st=>{
      const cells={};
      for(const date of dates){ cells[date]=attended.has(`${st.id}_${date}`) || attended.has(`${st.matricula}_${date}`) || attended.has(`${st.nombre}_${date}`) ? 'A' : 'NA'; }
...
function downloadRangeCSV(report){ const headers=['grupo','nombre',...report.dates]; const lines=[headers.join(',')]; for(const r of report.rows){ lines.push([r.grupo,r.nombre,...report.dates.map(d=>r.cells[d])].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')); } const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`asistencia_${report.group}_${report.start}_a_${report.end}.csv`; a.click(); }
function exportRangePDF(report){ const title=`Reporte de asistencia ${report.group}`; const head=report.dates.map(d=>`<th>${safe(formatDateShort(d))}</th>`).join(''); const body=report.rows.map(r=>`<tr><td>${safe(r.grupo)}</td><td>${safe(r.nombre)}</td>${report.dates.map(d=>`<td class="${r.cells[d]==='A'?'okCell':'naCell'}"><strong>${r.cells[d]}</strong></td>`).join('')}</tr>`).join(''); const w=window.open('', '_blank'); w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${safe(title)}</title><style>body{font-family:Arial,sans-serif;padding:22px;color:#111}h1{font-size:20px;margin:0 0 6px}p{margin:0 0 14px;font-size:12px}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #999;padding:5px;text-align:center}td:nth-child(2),th:nth-child(2){text-align:left}th{background:#eee}.okCell{color:#188038}.naCell{color:#d93025}@media print{@page{size:landscape;margin:12mm}button{display:none}}</style></head><body><h1>${safe(title)}</h1><p>Periodo: ${safe(report.start)} a ${safe(report.end)}</p><table><thead><tr><th>Grupo</th><th>Nombre</th>${head}</tr></thead><tbody>${body}</tbody></table><script>window.onload=()=>{window.print();}</script></body></html>`); w.document.close(); }
